...
[2026-04-09] Fixed prompt insertion race condition in agent conversations. When sending a manual message to an agent, the WebTerminal reconnected and sent a resize event while Claude CLI was still rendering its startup UI. The `claudePromptReady` check fired during the re-render, and the bracketed paste got garbled — Claude interpreted stray characters as keystrokes (e.g., typing `/exit` instead of the prompt). Fixed by debouncing `submitInitialPrompt`: instead of firing immediately on the first readiness match, it now resets a 500ms timer on each new output chunk, only submitting once the TUI output has settled. Also added bracketed paste mode to terminal-server.ts (was missing) and increased the hard fallback timer from 1.5s to 5s.
